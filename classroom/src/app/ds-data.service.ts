...
export function category_value(category: CategoryData | string, datum: DocumentData): number {
...
export function max_boxplot_value(data: DocuScopeData): number {
  ...
  if (data && data.categories) {
    maximum = data.categories.reduce((max, cat) => Math.max(max, cat.max, cat.uifence), 0.0);
  }
