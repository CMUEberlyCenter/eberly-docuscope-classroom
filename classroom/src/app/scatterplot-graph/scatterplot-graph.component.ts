...
export class ScatterplotGraphComponent implements OnInit, OnChanges {
  ...
  draw() {
    const ex_x = d3.extent(this.points.spdata, d => d['catX']);
    const ex_y = d3.extent(this.points.spdata, d => d['catY']);
    const max_value = Math.max(ex_x[1], ex_y[1]);
    ...
    const chart = d3.select('.scatterplot')
    ...
    const axis = d3.select('.scatterplot_axis').append('g');
    axis
    ...
    const spdata: ScatterplotDataPoint[] = this.points.spdata;
    const graph = axis.selectAll('.dot')
      .data(spdata);
    graph.exit().remove();
    graph
      .enter().append('circle')
